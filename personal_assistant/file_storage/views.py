...
def file_list(request):
    user = request.user
    query_images = File.objects.filter(owner=user).filter(datatype='image').order_by('name').all()
    query_videos = File.objects.filter(owner=user).filter(datatype='video').order_by('name').all()
    query_audios = File.objects.filter(owner=user).filter(datatype='audio').order_by('name').all()
    query_documents = File.objects.filter(owner=user).filter(datatype='document').order_by('name').all()
    query_other = File.objects.filter(owner=user).filter(datatype='other').order_by('name').all()
    data = {
        'images': query_images,
